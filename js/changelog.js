...
(function(){
  ...
  const entries = [
    {
      date: '2026-08-13',
      title: { zh: 'riverrun 混音器：性能优化（空闲降帧、移动端 DPR、低端降级、弱网友好）', en: 'riverrun mixer: performance optimizations (idle framerate, mobile DPR, low-end downgrade, weak-network friendly)' },
      body:  { zh: '按负载评估采纳全部四项优化。① 空闲降帧：RAF 循环改为调度式 loop()——播放中 60fps，未开始/已停止时降为 setTimeout 100ms（约 10fps）静态预览，消除页面可见但空闲时 60fps 重绘画布的持续耗电。② 移动端 DPR 限 1.5（原统一 2）：触屏设备画布像素减少约 44%，低端机重绘开销显著下降。③ 低端设备降级（navigator.hardwareConcurrency ≤ 4）：音轨 analyser fftSize 1024→512、跳过拾音连线绘制。④ 弱网友好：音频 preload 由 auto 改为 metadata，先取元数据秒开，播放时才拉流，避免 12 轨 17MB 预缓冲挤占带宽。全部改动仅 js/mixer-riverrun.js，语法构建通过。', en: 'Adopted all four optimizations from the load assessment. ① Idle framerate: the RAF loop became a scheduling loop() — 60fps while playing, dropping to setTimeout 100ms (~10fps) static preview when idle/stopped, eliminating the constant battery drain of 60fps canvas redraws while the page is visible but idle. ② Mobile DPR capped at 1.5 (was 2 everywhere): coarse-pointer devices redraw ~44% fewer pixels. ③ Low-end downgrade (navigator.hardwareConcurrency ≤ 4): track analysers drop fftSize 1024→512 and pickup-line strokes are skipped. ④ Weak-network friendly: audio preload changed from auto to metadata — metadata loads instantly, streams are pulled only on playback, so the 12-track 17MB prebuffer no longer competes for bandwidth. All changes confined to js/mixer-riverrun.js; syntax build passes.' },
      media: ''
    },
    {
      date: '2026-08-13',
      title: { zh: 'riverrun 混音器：移除鼠标单击行为（悬停即混音）', en: 'riverrun mixer: removed mouse click behavior (hover-only mixing)' },
      body:  { zh: '按需求删除鼠标单击的全部效果：pointerdown 的 mouse 分支改为直接 return（按下不再有任何作用），pointerup 不再重置鼠标麦克风（松开不再打断混音）。现在鼠标只有悬停/移动才混音（mouseMic 跟随光标），离开画布（pointerleave）才静音；单击、按住、松开均不介入。此前「松开后不动鼠标即静默」的怪癖随之消除——点击不再中断混音。触摸/手写笔逻辑不受影响（pointerdown 仍创建 touchMics，pointerup/cancel/lostpointercapture 仍清理）。', en: 'Removed the mouse click behavior entirely per request: the pointerdown mouse branch now returns early (pressing does nothing) and pointerup no longer resets the mouse mic (releasing no longer interrupts mixing). The mouse now only mixes through hover/movement (mouseMic follows the cursor) and mutes on pointerleave; clicks, presses and releases are inert. This also removes the old quirk where releasing the button and not moving the cursor left everything silent. Touch/stylus logic is unaffected (pointerdown still creates touchMics; pointerup/cancel/lostpointercapture still clean up).' },
      media: ''
    },
    {
      date: '2026-08-13',
      title: { zh: 'riverrun 混音器：FX 路永不静默（drone）+ 伪 reverse swell + 干声混响 + -12dB 约束', en: 'riverrun mixer: persistent drone FX + pseudo-reverse swell + dry reverb + -12dB constraint' },
      body:  { zh: '按需求强化 FX 链。① drone 永不静默：fxSend 恒有下限 0.08（原位置调制可到 0，导致 FX 路间断），fxFb 恒有 12–24%（delay 始终有 feedback），反馈环持续自哺，FX 路变成持续的 drone。② 伪 reverse（swell 包络）：锯齿 OscillatorNode → WaveShaper（y=(x+1)/2 斜坡整形）→ 慢升骤降包络，叠加 ConstantSourceNode（谷底 0.5–0.8）调制 drone 输出增益（swellGain，置于 fxDelay→fxReturn 之间，反馈环不受影响保持稳定），速率 0.18–0.63Hz、深度 0.2–0.5 随麦克风位置变化；听感为反向回声的「渐入」。③ 干声混响：master 另分 dryRevSend(0.15 固定) → 1.5s 合成 IR → limiter，避免干声太干。④ -12dB 动态约束：masterGain 与 fxReturn 各接 Analyser（fftSize 2048），frame 循环算 RMS 后 fxReturn.gain 以 setTargetAtTime(0.06) 收敛至 min(1, 0.251×dryRms/fxRms)，保证 FX 输出始终 ≤ 干声 -12dB，干声静默时压至 0.05。全部内置节点 + ConstantSourceNode（iOS Safari 兼容），无外部素材；修复了一个实现细节——swell 的直流基底最初误用无输入 GainNode（输出恒 0），改为 ConstantSourceNode。文档 9.2 节更新。', en: 'Strengthened the FX chain per request. ① Drone never silent: fxSend now has a permanent floor of 0.08 (previously position modulation could reach 0, gating the FX path off), fxFb is always 12–24% (delay always has feedback), and the feedback loop self-sustains — the FX path becomes a continuous drone. ② Pseudo-reverse (swell envelope): a sawtooth OscillatorNode → WaveShaper (y=(x+1)/2 ramp shaping) produces a slow-rise/quick-drop envelope, summed with a ConstantSourceNode (trough 0.5–0.8) modulating the drone output gain (swellGain placed between fxDelay and fxReturn so the feedback loop stays stable); rate 0.18–0.63 Hz and depth 0.2–0.5 follow the mic position — the sonic character of a reverse echo\'s build-up. ③ Dry reverb: a separate dryRevSend (fixed 0.15) → 1.5 s synthesized IR → limiter keeps the dry signal from being too dry. ④ -12 dB dynamic constraint: Analysers on masterGain and fxReturn (fftSize 2048); each frame RMS is computed and fxReturn.gain converges via setTargetAtTime(0.06) to min(1, 0.251×dryRms/fxRms), keeping the FX output at least 12 dB below the dry signal (dropped to 0.05 when dry is silent). All built-in nodes + ConstantSourceNode (iOS Safari compatible), no external assets; fixed an implementation detail — the swell DC offset initially used an unconnected GainNode (outputs constant zero), now a ConstantSourceNode. Section 9.2 docs updated.' },
      media: ''
    },
    {
      date: '2026-08-13',
      title: { zh: 'riverrun 混音器：全局并行发送 FX 链（混响背景，黑箱，参数随麦克风位置）', en: 'riverrun mixer: global parallel send FX chain (blurred reverb background, black-box, mic-position modulated)' },
      body:  { zh: '为 riverrun 混音器加入全局并行发送 FX 链，目标效果是产生「模糊的声音背景」。拓扑：masterGain 分出 fxSend（发送量由麦克风位置控制）→ 混响（ConvolverNode，2.6s 程序生成 IR，高 wet，无外部素材）→ 压缩（threshold -18 / ratio 6）→ EQ（peaking 520Hz ±3dB）→ 颤音（Gain + LFO，深度 4–10%，微弱）→ drive（WaveShaper tanh 曲线 + 前置增益 1.15–1.6，微弱）→ 延迟（0.28–0.62s）→ 少量反馈（6–24%）回混响前形成循环模糊 → 返回 limiter。黑箱：无任何 UI/HTML/CSS/i18n 改动，全部参数由麦克风位置在 frame 循环中调制——离点簇中心越近发送越多（背景越糊），簇内归一化位置 (ux,uy) 映射延迟时间/反馈/EQ/颤音速率与深度/drive 量，无麦克风时发送静默；用 setTargetAtTime 平滑免噪。IR 与驱动曲线均程序生成，全部为 Web Audio 内置节点（iOS Safari 兼容），12 轨 + 单条 FX 链 CPU 开销可忽略。文档 9.2 节同步更新。', en: 'Added a global parallel send FX chain to the riverrun mixer, aiming at a blurred/washed sound background. Topology: masterGain splits into fxSend (send amount controlled by the mic position) → reverb (ConvolverNode, 2.6 s procedurally generated IR, high wet, no external assets) → compressor (threshold -18 / ratio 6) → EQ (peaking 520 Hz ±3 dB) → tremolo (Gain + LFO, depth 4–10%, subtle) → drive (WaveShaper tanh curve + pre-gain 1.15–1.6, subtle) → delay (0.28–0.62 s) → small feedback (6–24%) back to before the reverb creating a looping blur → return into the limiter. Black box: no UI/HTML/CSS/i18n changes; all parameters are modulated in the frame loop by the microphone position — the closer the mic to the dot-cluster center, the more send (blurrier background); the normalized in-cluster position (ux, uy) maps delay time, feedback, EQ, tremolo rate/depth and drive amount; the send goes silent with no mic. setTargetAtTime gives click-free smoothing. The IR and drive curve are both generated in code; all nodes are built-in Web Audio (iOS Safari compatible); CPU cost of 12 tracks plus a single FX chain is negligible. Section 9.2 of the docs updated.' },
      media: ''
    },
    {
      date: '2026-08-13',
      title: { zh: 'riverrun 混音器：移除全部增益 UI、桌面收窄居中、说明列加宽并与交互框纵向对齐', en: 'riverrun mixer: removed all gain UI, desktop narrowed & centered, wider description aligned with the interaction box' },
      body:  { zh: '三项桌面端/整体调整。① 移除全部增益 UI：电脑和手机都不再显示增益控件（.mixer-gain 元素、CSS、JS 查询与监听、setActiveGain/updateGainReadout、mixerGainLabel i18n 一并删除），增益只由麦克风光标外圈半径 + 增益弧指示——桌面用滚轮（wheel）调节 mouseGain，手机固定 100%（touchGain），手写笔用 pressure。② 桌面横向收窄：.mixer-panel 改 flex-direction:column + max-width:1080px + margin:0 auto 居中（原为满宽左右分栏），标题横跨面板顶部；新增 .mixer-body 行容器承载舞台与说明列。③ 说明列加宽并与交互框纵向对齐：.mixer-desc 由 300px 增至 380px，与 .mixer-stage 同为 .mixer-body 的 flex 子项（align-items 默认 stretch），等高对齐；手机端 .mixer-body 转纵向、说明无边框随页面滚动不变。DOM 结构经浏览器 outerHTML 实测确认（mixer-header/mixer-body 就位、说明加载、无增益元素），全部 JS 通过语法构建。注：本机内存紧张（16GB 使用约 15GB），此前多次 headless Chrome 测试叠加导致 macOS OOM 杀掉 VSCode（"killed"），后续改为静态验证不再启重型测试。', en: 'Three adjustments, mostly desktop. ① All gain UI removed: neither desktop nor mobile shows a gain control anymore (.mixer-gain element, CSS, JS queries/listeners, setActiveGain/updateGainReadout, and the mixerGainLabel i18n key all deleted); gain is indicated only by the mic-cursor outer ring radius and gain arc — desktop adjusts mouseGain with the scroll wheel, mobile is fixed at 100% (touchGain), stylus uses pressure. ② Desktop narrowed: .mixer-panel becomes flex-direction:column with max-width:1080px and margin:0 auto (previously full-width row split); the title spans the top of the panel; a new .mixer-body row container holds the stage and the description. ③ Description widened and vertically aligned with the interaction box: .mixer-desc grows from 300px to 380px and, as a flex child of .mixer-body alongside .mixer-stage (default align-items:stretch), they are equal-height aligned; mobile keeps the column stacking with a borderless description that scrolls with the page. The DOM was confirmed via browser outerHTML dump (mixer-header/mixer-body in place, description loaded, no gain element); all touched JS builds cleanly. Note: this machine is memory-tight (16GB with ~15GB in use); repeated headless-Chrome test runs previously piled up and macOS OOM-killed VSCode ("killed"), so verification switched to static checks without heavy test processes.' },
      media: ''
    },
    {
      date: '2026-08-13',
      title: { zh: 'riverrun 混音器：手机增益固定 100%、删除增益提示、说明无边框随页滚动', en: 'riverrun mixer: mobile gain fixed at 100%, gain hint removed, description unframed and page-scrolled on mobile' },
      body:  { zh: '三项手机端调整。① 增益固定为 100%：touchGain 默认值由 0.7 改为 1.0（触摸麦克风增益恒满，实测麦克风光标外圈半径 23.6px 与 100% 理论值 21.9px 吻合）。② 删除增益提示：手机端（isCoarse）启动时不再显示 .mixer-gain 控件（start() 中 gainWrap.style.display 按 isCoarse 置 none），清除移动端残留的增益样式；桌面端分段条保留。③ 手机端说明不再框起来：.mixer-panel 改 height:auto + overflow:visible（min-height:100dvh），.mixer-desc 无边框、透明背景、随页面自然滚动（实测 scrollHeight 1881 > viewport 757），与 wwhbh/ecce 等布局的手机行为一致。STYLEGUIDE 7f 与程序编写说明.md 9.4 同步更新。', en: 'Three mobile adjustments. ① Gain fixed at 100%: touchGain default changed from 0.7 to 1.0 (touch-mic gain is always full; measured mic-cursor ring radius 23.6px matches the 100% theoretical 21.9px). ② Gain hint removed: on coarse devices start() no longer shows the .mixer-gain control (gainWrap.style.display set to none by isCoarse), stale mobile gain styles cleaned up; the desktop segmented bar stays. ③ Mobile description is no longer framed: .mixer-panel becomes height:auto + overflow:visible (min-height:100dvh) and .mixer-desc loses its border and background, flowing with natural page scroll (measured scrollHeight 1881 > viewport 757), matching the mobile behavior of the wwhbh/ecce layouts. STYLEGUIDE 7f and section 9.4 of 程序编写说明.md updated.' },
      media: ''
    },
    {
      date: '2026-08-13',
      title: { zh: 'riverrun 混音器：手机端固定增益、交互区缩小、移除标题副标题', en: 'riverrun mixer: fixed gain on mobile, smaller interaction area, removed title subtitle' },
      body:  { zh: '三项手机端 UI 调整。① 增益条改为固定增益：手机端（max-width:768px）隐藏 −/＋ 按钮（.mixer-gain-btn display:none），.mixer-gain 退化为简单读数（padding:7px 10px），触摸麦克风增益恒为 70%；桌面端分段条保留（滚轮/按钮均可调）。② 手机交互区纵向尺寸从 50dvh 减至 40dvh，舞台更紧凑，给下方说明列更多空间。③ 移除 riverrun 标题下的副标题（.mixer-hint 元素、样式与 mixerHint i18n 键一并删除）。无头浏览器实测：手机舞台 303px≈40dvh、增益按钮 display:none、读数 70%、无小标题；桌面舞台全高 651px、分段条按钮可见、滚轮 80→88% 正常。', en: 'Three mobile UI adjustments. ① Gain became fixed on mobile: below max-width:768px the −/+ buttons are hidden (.mixer-gain-btn display:none) and .mixer-gain degrades to a plain readout (padding:7px 10px), with touch-mic gain fixed at 70%; the desktop segmented bar remains (wheel/buttons both work). ② The mobile interaction stage shrank from 50dvh to 40dvh, giving the description column below more room. ③ Removed the subtitle under the riverrun title (the .mixer-hint element, its styles, and the mixerHint i18n key). Verified in headless Chrome: phone stage 303px ≈ 40dvh, gain buttons display:none, readout 70%, no subtitle; desktop stage full-height 651px, segmented-bar buttons visible, wheel 80→88% still works.' },
      media: ''
    },
    {
      date: '2026-08-13',
      title: { zh: 'riverrun 混音器：双栏布局重构（左交互/右说明，手机上下）+ 增益控件重设计', en: 'riverrun mixer: two-column layout (interaction left / description right, stacked on mobile) + gain control redesign' },
      body:  { zh: '重构 riverrun 混音器布局并重设计增益控件。① 布局：作品说明不再折叠进 INFO 浮层，改为内嵌说明列——桌面端 .mixer-panel 改为 flex-direction:row，左侧 .mixer-main（标题 + 舞台）占 flex:1，右侧 .mixer-desc（width:300px，3px 边框，overflow-y:auto）展示完整描述与 related 互链；手机端改回 column，舞台固定 height:50dvh 在上、说明列 flex:1 在下可滚动。移除 INFO 按钮与 #mixer-info 浮层（HTML/CSS/JS 一并清理），getDescEl() 的 mixer 分支改为返回 #mixer-desc。② 空白区域：点簇格距由 min(W,H)*0.20 增大到 0.26（拾音半径同步 0.26），实测桌面点簇 512×349 占交互区 56%×56%、手机 64% 宽，不再大片留白。③ 增益控件重设计：从分散的小方钮改为紧凑分段条 [− | GAIN 70% | +]——按钮与容器齐平无外边距、之间 2px 分隔线、数值居中加粗；触屏端整条高 40px、按钮 40×40 满足触控目标。更新 mixerHint 文案与 STYLEGUIDE 7f / 程序编写说明.md 9 节。无头浏览器实测：桌面 panelFlex=row 交互在左说明在右、手机 column 交互在上说明在下，说明正常加载，START/增益按钮/双指触摸/滚轮全部正常。', en: 'Restructured the riverrun mixer layout and redesigned the gain control. ① Layout: the work description no longer hides behind an INFO overlay — it is now an inline description column. On desktop .mixer-panel becomes flex-direction:row with the left .mixer-main (title + stage) at flex:1 and the right .mixer-desc (width:300px, 3px border, overflow-y:auto) showing the full description and related links; on mobile it returns to column — stage fixed at height:50dvh on top, description flex:1 and scrollable below. The INFO button and #mixer-info overlay were removed (HTML/CSS/JS cleaned up); getDescEl() mixer branch now returns #mixer-desc. ② Blank space: the dot cluster grid spacing grew from min(W,H)*0.20 to 0.26 (pickup radius synced to 0.26); measured cluster 512×349 fills 56%×56% of the interaction area on desktop and 64% width on mobile — no more large empty margins. ③ Gain control redesign: the scattered small square buttons became a compact segmented bar [− | GAIN 70% | +] — buttons flush with the container (no margins), 2px dividers between segments, centered bold value; on coarse pointers the whole bar is 40px tall with 40×40 tap targets. Updated the mixerHint copy and STYLEGUIDE 7f / section 9 of 程序编写说明.md. Verified in headless Chrome: desktop panelFlex=row (interaction left, description right), mobile column (interaction top, description bottom), description loads, START / gain buttons / multi-touch / wheel all work.' },
      media: ''
    },
    {
      date: '2026-08-13',
      title: { zh: 'riverrun 混音器：手机端增益控件（+/−）', en: 'riverrun mixer: on-screen gain control (+/−) for mobile' },
      body:  { zh: '为 riverrun 混音器增加屏幕增益控件，解决手机端无法用按压控制增益的问题。背景：用户询问手机按压（PointerEvent.pressure）能否控制增益——结论是硬件限制：iPad + Apple Pencil、S-Pen 等主动触控笔有真实压力传感器，代码本就支持（pen 指针走 pressure）；但现代手机的手指没有力度传感器（3D Touch 自 iPhone 11 起移除，Android 亦无），pressure 恒为 0.5，无法用按压控制。实现：HUD 增益读数两侧新增 − / ＋ 按钮（.btn-mixer-gain，触屏设备经 @media (pointer: coarse) 放大到 36px 便于点击）；手机端调节全局 touchGain（新变量，默认 0.7）并实时同步所有现有触摸麦克风，新触摸直接取该值；桌面端按钮等效于滚轮（调 mouseGain），滚轮功能保留；手写笔仍用真实 pressure。micGain(e) 统一分派：mouse→mouseGain、pen→pressure、touch→touchGain；updateGainReadout 按当前设备显示对应增益与多麦克风计数（×N）。无头浏览器实测：手机模拟点 + 70%→80%→90%、点 − 回落，双指触摸读数 80%·×2；桌面滚轮 80%→88% 正常。', en: 'Added an on-screen gain control (+/−) to the riverrun mixer to solve gain adjustment on phones. Background: the user asked whether press force (PointerEvent.pressure) can control gain on mobile — the answer is a hardware limitation: active styluses (Apple Pencil on iPad, Samsung S-Pen) have real pressure sensors and were already supported (pen pointers use pressure), but modern phones have no finger force sensor (3D Touch was removed after iPhone 11; Android phones lack it too), so pressure is always 0.5 and cannot control gain. Implementation: − / ＋ buttons flank the HUD gain readout (.btn-mixer-gain; enlarged to 36px on coarse pointers via @media (pointer: coarse) for easy tapping); on phones they adjust a new global touchGain (default 0.7) and live-sync all active touch mics, new touches simply use that value; on desktop the buttons are equivalent to the wheel (adjusting mouseGain) and the wheel still works; styluses keep real pressure. micGain(e) now dispatches: mouse→mouseGain, pen→pressure, touch→touchGain; updateGainReadout shows the device-appropriate gain and the multi-mic count (×N). Verified in headless Chrome: phone simulation + goes 70%→80%→90%, − steps back, two-finger touch reads 80%·×2; desktop wheel still works (80%→88%).' },
      media: ''
    },
    {
      date: '2026-08-13',
      title: { zh: 'riverrun 混音器修复：手机端无操作时音轨自动播放', en: 'riverrun mixer fix: tracks auto-played on mobile without any interaction' },
      body:  { zh: '修复 riverrun 空间混音器在手机端的严重问题：点击「开始混音」后不做任何操作，12 条音轨便全部自动播放。根因：iOS/Android 在触摸时会派发「兼容性鼠标」指针事件（pointerType=\'mouse\'），原代码将其当作真正的鼠标悬停，把鼠标麦克风 mouseMic 钉在触摸点（屏幕中央，恰是紧凑点簇所在处），手指抬起后该事件没有对应的释放，幽灵麦克风持续存在，使全部音轨以近满音量混音。修复分两层：① 触摸设备门控——先用 matchMedia(\'(pointer: fine)\') 判断并禁用鼠标麦克风，但实测发现 iOS Safari 会同时报 coarse 与 fine（matchMedia 不可靠），遂改为 navigator.maxTouchPoints > 0 作为可靠标志（iPhone=5/iPad=10），与 matchMedia 取或：pointerdown/pointermove 的 mouse 分支均以 !isCoarse 门控，activeMics() 汇总时直接排除 mouseMic；② 防御层——启动完成瞬间 touchMics.clear() 清除启动序列可能残留的幽灵触摸；增加 lostpointercapture 与 window blur 处理器（iOS 系统手势会丢 pointerup / 切走应用时清空所有麦克风）。触摸麦克风（touchMics，按 pointerId 索引）不受影响；桌面端鼠标悬停拾音、滚轮调增益完全保留。另将 scripts/server.py 升级为 ThreadingHTTPServer（单线程版曾被一条卡死的客户端连接阻塞导致整个站点超时无法访问）并为 HTML/JS/CSS 增加 Cache-Control: no-cache（音频 m4a 保持可缓存）；mixer 脚本引用加 ?v=3 强制手机刷新。无头浏览器实测（maxTouchPoints=5 + matchMedia 谎报 fine:true 的最坏情况）：兼容性鼠标事件不产生麦克风（0 像素）；触摸正常（按下 284px → 抬起归零）；结束后无幽灵麦克风；桌面对照组鼠标悬停仍激活（217px）。', en: 'Fixed a severe issue on mobile in the riverrun spatial mixer: after tapping START, all 12 tracks auto-played with no interaction. Root cause: iOS/Android dispatch "compatibility mouse" pointer events (pointerType=\'mouse\') during touches; the code treated these as real mouse hovers and parked the mouseMic at the touch point (screen center — exactly where the compact dot cluster is). The compatibility pointer has no corresponding release after the finger lifts, so a ghost mic persisted and drove every track near full volume. Two layers of fix: ① touch-device gating — first used matchMedia(\'(pointer: fine)\'), but testing revealed iOS Safari reports both coarse and fine (unreliable), so it was switched to navigator.maxTouchPoints > 0 (iPhone=5 / iPad=10) ORed with matchMedia; the pointerdown/pointermove mouse branches are gated by !isCoarse and activeMics() excludes mouseMic outright; ② defensive layers — touchMics.clear() the instant start completes (wipes any ghost touch from the start sequence), plus lostpointercapture and window blur handlers (iOS system gestures can drop pointerup; switching apps/locking clears all mics). Touch mics (touchMics, keyed by pointerId) are unaffected; desktop hover mixing and scroll-wheel gain remain fully intact. Also upgraded scripts/server.py to ThreadingHTTPServer (the single-threaded version was blocked by one stalled client connection, taking the whole site down) and added Cache-Control: no-cache for HTML/JS/CSS (audio m4a stays cacheable); the mixer script tag now carries ?v=3 so phones are forced to refetch. Verified in headless Chrome under the worst case (maxTouchPoints=5 + matchMedia lying fine:true): compatibility mouse events create no mic (0 pixels), real touches still work (284px on press → 0 on release), no ghost mic afterwards; the desktop control still activates the hover mic (217px).' },
      media: ''
    },
    {
      date: '2026-08-13',
      title: { zh: 'riverrun 作品页：交互式空间混音器（复现 The FET Mixer）', en: 'riverrun work page: interactive spatial mixer (recreating The FET Mixer)' },
      body:  { zh: '为 riverrun 作品页实现了交互式空间混音器，在网页上复现 The FET Mixer 的核心交互逻辑。12 条《芬尼根的守灵夜》有声书音轨在二维面板上排布为带编号的黑点（4 列 × 3 行），点半径随各轨响度脉动；所有音轨同步循环播放，用户以光标（虚拟麦克风，抽象指示：十字 + 虚线外圈 + 增益弧）在面板内移动，离某轨越近该轨混音音量越大（smoothstep 距离衰减）。原始素材为 12 条 2ch/48kHz/Int16 PCM WAV（各 133 秒、等长，设计为同步循环），用 macOS 内置 afconvert 转为 AAC-LC m4a（约 92 kbps，每条约 1.5 MB，共约 17 MB）部署于 audio/riverrun/1.m4a～12.m4a。音频采用 12 个 MediaElementAudioSourceNode（流式、低内存），每轨 source → Analyser(响度可视化) → Gain(距离混音) → StereoPanner(按 X 固定声像) → masterGain → limiter(压缩限幅) → destination；用户点击 START 后在手势内 play()+pause() 解锁（iOS 要求），待全部 canplaythrough 后统一 currentTime=0 并紧密连发 play() 同步起播。关于用户提问的两点：① 多个麦克风可行——多点触控时每根手指即一只麦克风（PointerEvent 按 pointerId 索引），其贡献以饱和求和 1-∏(1-g) 叠加，忠实于 The FET Mixer 的多输出概念；鼠标为常驻一只。② 麦克风增益控制部分可行——触控/手写笔/Force Touch 设备读取 PointerEvent.pressure（按压力度）作为增益，但标准鼠标无压力数据，故回退为滚轮调节（wheel，Shift 微调），光标外圈半径与增益弧实时反映增益。新增 mixer 布局：js/mixer-riverrun.js（App.initRiverrunMixer / App.refreshRiverrunMixer，与 WWHBH 的 initMicButton/refreshMicButton 同构），js/project-data.js 中 riverrun 改 layout:\'mixer\' 并加 audioDir/tracks 字段，js/project.js 的 layoutMap/getDescEl/fillContent 增加 mixer 分支并在初始化与语言切换时调用 init/refresh，js/project-i18n.js 增加 mixerStart/mixerStop/mixerLoading/mixerInfo/mixerClose/mixerGainLabel/mixerHint 双语文案，css/project.css 增加 .mixer-panel 等样式（全屏 Canvas 面板 + 居中 START 覆盖 + 右上角 STOP/INFO/增益读数 + INFO 浮层载入完整描述与 related 互链），project-template.html 增加 layout-mixer 面板与脚本引用。UI 启动前画静态点阵预览，启动后右上角出现 STOP/INFO 与增益读数，INFO 浮层可开关；visibilitychange 隐藏标签页时暂停全部保持相对同步。文档同步：程序编写说明.md 新增第 9 节并重编号 9-16，STYLEGUIDE.md 新增 7f Mixer 布局。', en: 'Built an interactive spatial mixer for the riverrun work page that recreates the core interaction logic of The FET Mixer on the web. 12 audiobook tracks of Finnegans Wake are laid out as numbered black dots on a 2D panel (4 columns × 3 rows), their radii pulsing with each track\'s loudness; all tracks play back in sync and loop, and the user moves a cursor (a virtual microphone, shown as an abstract indicator: crosshair + dashed ring + gain arc) across the panel — the closer to a track, the louder it is in the mix (smoothstep distance falloff). The source material was 12 2ch/48kHz/Int16 PCM WAV files (133 s each, equal length, designed to loop in sync), converted to AAC-LC m4a with the built-in macOS afconvert (~92 kbps, ~1.5 MB each, ~17 MB total) and deployed as audio/riverrun/1.m4a–12.m4a. Audio uses 12 MediaElementAudioSourceNodes (streaming, low memory); each track runs source → Analyser(loudness viz) → Gain(distance mixing) → StereoPanner(fixed pan by X) → masterGain → limiter(compression) → destination. On START, each <audio> is unlocked with play()+pause() within the user gesture (iOS requirement), then once all reach canplaythrough they are all set to currentTime=0 and play() is fired in a tight loop for synchronous start. Regarding the two questions asked: ① multiple microphones are feasible — on multi-touch each finger is one microphone (PointerEvent indexed by pointerId), their contributions combined by saturating summation 1-∏(1-g), faithful to The FET Mixer\'s multi-output concept; the mouse is one persistent mic. ② microphone gain control is partially feasible — touch/pen/Force-Touch devices read PointerEvent.pressure (press force) as the gain, but a standard mouse reports no pressure, so it falls back to the scroll wheel (wheel event, Shift for fine steps); the cursor\'s outer ring radius and gain arc reflect the gain in real time. A new mixer layout was added: js/mixer-riverrun.js (App.initRiverrunMixer / App.refreshRiverrunMixer, isomorphic to WWHBH\'s initMicButton/refreshMicButton); js/project-data.js switches riverrun to layout:\'mixer\' with audioDir/tracks fields; js/project.js adds a mixer branch to layoutMap/getDescEl/fillContent and calls init/refresh on init and language toggle; js/project-i18n.js adds bilingual strings (mixerStart/mixerStop/mixerLoading/mixerInfo/mixerClose/mixerGainLabel/mixerHint); css/project.css adds .mixer-panel styles (full-screen Canvas panel + centered START overlay + top-right STOP/INFO/gain readout + INFO overlay loading the full description and related links); project-template.html adds the layout-mixer panel and script include. Before start a static dot preview is drawn; after start STOP/INFO and a gain readout appear top-right; the INFO overlay is toggleable; visibilitychange pauses all tracks when the tab is hidden to preserve relative sync. Docs updated: added section 9 to 程序编写说明.md and renumbered 9–16; added 7f Mixer layout to STYLEGUIDE.md.' },
