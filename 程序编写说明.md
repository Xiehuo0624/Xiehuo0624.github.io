...
- **全局并行发送 FX 链（黑箱，无 UI）**：从 `masterGain` 分出发送与干声混响——① **干声混响**：`dryRevSend(0.15 固定) → 1.5s IR → limiter`，避免干声太干；② **drone FX 链**：`fxSend(0.08–0.5，恒有下限) → 混响(2.6s 合成 IR, 高wet) → 压缩(threshold -18/ratio 6) → EQ(peaking 520Hz ±3dB) → 颤音(深度 4–10%, 微弱) → drive(tanh 曲线 + 前置增益 1.15–1.6, 微弱) → 延迟(0.28–0.62s) → fxFb(恒有 12–24%)回混响前形成循环 → swell(伪 reverse 包络) → fxReturn → limiter`。**drone 永不静默**：send/feedback 恒有下限，由反馈环持续自哺；**伪 reverse**：锯齿 LFO 经斜坡整形（WaveShaper y=(x+1)/2）得到慢升骤降包络，调制 drone 输出（谷底 0.5–0.8、深度 0.2–0.5、速率 0.18–0.63Hz），听感如反向回声的「渐入」；**干声包络跟随**：frame 循环用双 Analyser 测干声与 FX 的 RMS，单极点包络跟随器（上升 attack 50ms / 下降 decay 150ms，满度参考 0.2 RMS）直接控制 FX 输出音量 `fxReturn.gain = 0.5×包络`（约 -6dB，按用户反馈从 -12dB 放宽以提高可闻度），闭路约束（fx ≤ 干声×0.5）作硬顶兜底——drone 随干声动态呼吸，起音快、衰减慢。所有参数由麦克风位置黑箱调制，且**每次启动随机生成 3 个高反馈「共振点」**（`makeFxZones()`，与音轨洗牌同机制）：面板上随机放置 3 个音色点（黑箱不绘制，位置收拢于中部 0.30–0.70），反馈略高于中性基础（0.24–0.36 vs 0.18），三个音色原型——①暗长（长延迟 0.6–0.8s + 低频衰减 EQ + 慢颤音 + 深 swell）、②亮闪（短延迟 0.15–0.28s + 高频提升 EQ + 快颤音 + 浅 swell + 更多 drive）、③中雾（中延迟 + 平 EQ）——各自在区间内随机取值并随机分配位置；三个效果圈的拾取半径保证互相重叠（直径 = 最大两两间距 × 1.25，随窗口缩放），麦克风几乎始终处于音色混合场中：靠近某点即该点音色主导（距离 smoothstep 加权混合）。无外部素材（IR 与驱动/斜坡曲线均程序生成），全部内置节点 + ConstantSourceNode，iOS Safari 兼容。
