...
- **全局并行发送 FX 链（黑箱，无 UI）**：从 `masterGain` 分出发送与干声混响——① **干声混响**：`dryRevSend(0.15 固定) → 1.5s IR → limiter`，避免干声太干；② **drone FX 链**：`fxSend(0.08–0.5，恒有下限) → 混响(2.6s 合成 IR, 高wet) → 压缩(threshold -18/ratio 6) → EQ(peaking 520Hz ±3dB) → 颤音(深度 4–10%, 微弱) → drive(tanh 曲线 + 前置增益 1.15–1.6, 微弱) → 延迟(0.28–0.62s) → fxFb(恒有 12–24%)回混响前形成循环 → swell(伪 reverse 包络) → fxReturn → limiter`。**drone 永不静默**：send/feedback 恒有下限，由反馈环持续自哺；**伪 reverse**：锯齿 LFO 经斜坡整形（WaveShaper y=(x+1)/2）得到慢升骤降包络，调制 drone 输出（谷底 0.5–0.8、深度 0.2–0.5、速率 0.18–0.63Hz），听感如反向回声的「渐入」；**-12dB 动态约束**：frame 循环里用双 Analyser 测干声与 FX 的 RMS，`fxReturn.gain = min(1, 0.251×dry/fx)` 平滑收敛，保证 FX 输出始终 ≤ 干声 -12dB。所有参数由麦克风位置黑箱调制，且**每次启动随机生成映射画像**（`makeFxMap()`，与音轨洗牌同机制）：每个参数随机选择坐标（ux / uy / 径向 r）与斜率方向（正/负），因此每次混音的效果器响应方式都不同；无麦克风时保持下限。无外部素材（IR 与驱动/斜坡曲线均程序生成），全部内置节点 + ConstantSourceNode，iOS Safari 兼容。
