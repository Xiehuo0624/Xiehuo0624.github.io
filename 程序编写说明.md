...
<script src="js/mixer-riverrun.js"></script> <!-- 9. riverrun 空间混音器 App.initRiverrunMixer -->
<script src="js/project.js"></script>       <!-- 10. 项目页主逻辑（最后加载，可引用上述所有） -->
```
...
- **增益控制**：分设备策略——手写笔（Apple Pencil / S-Pen）读取 `PointerEvent.pressure` 真实压感；鼠标用滚轮（`wheel` 事件，Shift 微调）；手机手指在 HUD 用屏幕 − / ＋ 按钮调节（新增 `touchGain`，实时同步所有现有触摸麦克风）。原因：现代手机没有手指力度传感器（3D Touch 自 iPhone 11 起移除），`pressure` 恒为 0.5，按压控制不可行；手写笔是唯一有真实压感的常见触控设备。光标外圈半径与增益弧长度实时反映增益。
