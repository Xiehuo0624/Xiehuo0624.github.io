...
9. [riverrun 空间混音器（The FET Mixer 交互复现）](#9-riverrun-空间混音器the-fet-mixer-交互复现)
...
<script defer src="js/audio-wwhbh.js?v=2"></script>   <!-- 9. WWHBH 音频 App.initMicButton -->
<script defer src="js/mixer-riverrun.js?v=4"></script> <!-- 10. riverrun 混音器 App.initRiverrunMixer -->
<script defer src="js/project.js?v=2"></script>        <!-- 11. 项目页主逻辑 -->
```
...
- 非安全上下文（`window.isSecureContext === false` 或缺少 `mediaDevices/getUserMedia`）在请求权限前直接短路，按钮显示「需要 HTTPS」并 2 秒后恢复；
- `getUserMedia` 需用户授权，失败时按钮显示「权限被拒」并 2 秒后恢复；
- `running` + `starting` 双状态标志：运行中再次点击执行关闭；权限/初始化进行中忽略重复点击，避免双击创建两套不可控的麦克风回授图；
- `stop()` 函数断开所有节点、停止麦克风轨道、关闭 AudioContext，彻底释放资源；
- 语言切换时 `App.refreshMicButton()` 更新按钮文案（启动/关闭/权限被拒/需要 HTTPS）。

### 8.4 i18n 驱动的按钮文案

按钮文本不硬编码，而是通过 `App.I18n.t()` 动态读取，支持四种状态的多语言：
...
## 9. riverrun 空间混音器（The FET Mixer 交互复现）

作品 riverrun 是使用 The FET Mixer 创作的交互式声音装置：12 条《芬尼根的守灵夜》有声书音轨铺设在乐器面板上，观众移动传感器在面板上拾取电磁波进行混音。网站在 riverrun 作品页以 Web Audio + Canvas 复现其核心交互逻辑，新增 `mixer` 布局。
...
`js/project.js` 的 `layoutMap` 增加 `mixer:'layout-mixer'`，`getDescEl()` 对 mixer 返回 `#mixer-desc`（描述载入内嵌说明列——桌面在交互右侧、手机在交互下方，不再折叠进浮层），并在 `fillContent()` 的 mixer 分支填充标题、为小写标题作品加 `lowercase` 类；初始化与语言切换时分别调用 `App.initRiverrunMixer()` 与 `App.refreshRiverrunMixer()`（与 WWHBH 的 `initMicButton`/`refreshMicButton` 同构）。
...
- **多麦克风**：多点触控时每根手指即一只麦克风（`PointerEvent` 按 `pointerId` 索引），贡献以饱和求和叠加——忠实于 The FET Mixer 的多输出概念。鼠标为常驻一只（光标进入面板即拾音、离开即静音）。
...
5. **Web Audio API**：用长延时节点与反馈增益节点模拟 WWHBH 的麦克风-扬声器回授机制；以 12 轨 MediaElementSource + 距离衰减 + 多麦克风饱和求和复现 riverrun / The FET Mixer 的空间混音交互；
