#!/usr/bin/env python3
"""作品页文案风格探针 —— 量出「这段中文还像不像作者自己写的」。

## 为什么需要它

2026-09-23 的《作品页人味审计》（`docs/作品页人味审计.md`）逐段读了三件按
「创作年份／形态／规格」标准重写的作品页，发现它们的**标点与语气特征**和作者
自己的手笔差出一个量级。最干净的一项是破折号「——」：

| 组 | 「——」密度 | 语气词密度 |
|---|---:|---:|
| 作者原笔（wwhbh ＋ edgedgedge ＋ 两份 docx，5,336 汉字） | **0.37‰**（2 次） | 3.00‰ |
| 未重写的另外几件（器材清单体／bug 记录，作者手笔） | 最高 1.78‰ | 0–0‰ |
| 本次重写的三件 | **4.72 / 4.71 / 5.47‰** | 0 / 0 / 0.61‰ |

重写三件就是全站前三名，与原笔之间**没有交叉**。所以「——」密度可以当一条
**可复算的漂移信号**：它不判断文笔好坏，只回答「这段中文的标点习惯是不是换人了」。

阈值取 **3.0‰**：原笔 0.37、未重写最高 1.78，重写最低 4.71 —— 3.0 落在两簇
正中间，两边各留出 1.2 以上的余量。

## 它不做什么

- **不判断人味**。审计 §1 的结论是：人味来自段落的**经验来源**（有没有一件发生
  过的事），那只能靠人读。本探针只看可算的部分。
- **语气词那一列不是判据**。器材清单体的 The JustType Study 与 bug 记录的
  SPECTRAL DISSECTOR 同样是 0‰ —— 那也是作者写的，只是不在叙述。它分的是
  「叙述体 vs 论述体」，不是「作者 vs 别人」。
...
def main():
    ...
    if over:
        ...
        print('      作者原笔基线 0.37‰。这一段可能不是你的标点习惯，'
              '见 docs/作品页人味审计.md §1、附录 B。')
        if args.check:
            return 1
    else:
        print(f'\n「——」密度全部在 {DASH_WARN}‰ 以内。')
