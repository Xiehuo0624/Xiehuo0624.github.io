...
5. **文案风格**（2026-09-23 立）：改了作品页**中文正文**后，另跑一次 `python3 scripts/style-probe.py`。
   它把每页的「——」密度对**作者原笔基线**（wwhbh ＋ edgedgedge ＋ `docs/` 下两份 docx，
   5,336 汉字，0.37‰）作比，超过 **3.0‰** 即告警：2026-09-20 按标准重写的那三件是
   4.72／4.71／5.47‰，而作者自己写的页面从不超过 1.78‰ —— 这是「这段中文换人写了」最干净的信号。
   **它是观察工具，不是上线门槛**（不加 `--check`，不影响提交），不需要本地服务与 Chrome。
   判据、它不判断什么，见 `docs/作品页人味审计.md` 与脚本文件头。
