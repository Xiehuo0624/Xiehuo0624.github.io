...
### 4a. 文案迭代期不跑字形链（2026-09-24 立）

**文本还在改的时候，一条检查都不要跑；作者说定稿，再一次性跑完下面 1–5 全部动作。**

| 时机 | 只做什么 |
|------|----------|
| 文案迭代期 | 改 `data/*/*.html`／`*-i18n.js`；要预览就起本地服务用眼睛看 |
| 作者说定稿 | 按 1–5 走完：缓存号 → changelog（先全文、跑 split、再压 brief）→ 字体链 → 生成器 → verify + coverage → style-probe |

理由（**2026-09-24 实测**，不是推测）：

- **字体链本身不是负担**：`gen-cjk-main.py` 走完整条链只要 **2.2 秒**，而且幂等（内容没变就一个字节不改）。
  真正的重复工作量是两个 Chrome 探针 —— `coverage.mjs` 一遍 **70 秒**，`verify.mjs` 653 项断言要几分钟。
- **推迟在技术上是安全的**：不重跑字体链时，`css/base.css` 与各 HTML 里的字体 URL **彼此仍然一致**（都是旧号），
  `verify.mjs` 第十二节照样通过。唯一的缺口是「新打的字没有字形」，而那正是 `coverage.mjs` 该抓的。
- **顺序不能倒**：`js/changelog.js` 在字体推导范围内，且 `js_strings()` 会把 `body` 全文一起算进去。
  若先跑字体链再写 changelog，条目里的新字仍会漏掉；若带着 `body` 全文跑，则会把**不部署**的全文用字
  也算进子集、白白撑大字体。所以必须**先写全文 → 存档 → 压成 brief → 才切字体**。

**这条规矩的前提是探针可信**。`coverage.mjs` 原先只 `sleep(1200)` 就抓 `innerText`，而 `/works/*`
的正文是 fetch 后异步注入的，于是**同一份内容**实测给出 8／20／36／0 四种结果，还出现过正文一个字都没填
却记「0 缺字」通过的假绿；CDP 不回话时更会整遍挂死（30.5s、零输出、退出码 0）。2026-09-24 已修：
每条 CDP 调用带 15s 超时，抓文本前走 `settled()` 确认正文已注入且 `innerText` 收敛，未就绪的页
记失败并退出码 1，**绝不落进「0 缺字」那一档**。修完连跑三遍结果一致。

**多会话并发**（2026-09-25 补）：作者会同时开多个会话改这个仓库（实测两个会话在 18:04–18:23 之间
交替写 `data/riverrun/*`、`js/changelog.js`、字体与 `css/base.css`）。此时**两个探针会互相作废**：
一个会话跑完 verify，另一个会话接着改文案，那份「全绿」就不再成立。所以——
① 并发期各方都按上面的 4a 办：改文案的只管改，**由定稿的那一方跑收尾链**；
② `coverage.mjs` 原先调试端口与 `user-data-dir` 都写死，两个会话同跑时第二个 Chrome 绑不上端口、
又因 profile 被锁直接退出，脚本会读到**上一次**的 `DevToolsActivePort` 去驱动**第一个会话的浏览器**
